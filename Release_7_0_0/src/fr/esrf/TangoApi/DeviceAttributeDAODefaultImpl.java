...
public class DeviceAttributeDAODefaultImpl implements IDeviceAttributeDAO
{
	 ...
	 *
	 *	@param attrval	AttributeValue_4 input object
	 */
	//===========================================
	...
	{
		attrval.data_format = AttrDataFormat.FMT_UNKNOWN;
		attrval.err_list = new DevError[0]; // ?? Only for DevEncoded, else (NullPointerException)
	...
	public short[] extractUCharArray() throws DevFailed
	{
		...
		if (use_union)
		{
			byte[]	argout = attrval.value.uchar_att_value();
	...
	public int[] extractUShortArray() throws DevFailed
	{
		...
		if (use_union)
		{
			short[]	argout = attrval.value.ushort_att_value();
	...
	public long extractULong() throws DevFailed
	{
		...
		if (use_union)
		{
			int[]	argout = attrval.value.ulong_att_value();
			long	mask = (long) 0x7fffffff;
			mask += ((long)1 << 31);
			return (mask & (long)argout[0]);
		}
		else
	...
	public long[] extractULongArray() throws DevFailed
	{
		...
		if (use_union)
		{
			int[]	array = attrval.value.ulong_att_value();
			long	mask = (long) 0x7fffffff;
			mask += ((long)1 << 31);
...
.	 *		SPECTRUM, IMAGE or FMT_UNKNOWN)
	...
	public int getType() throws DevFailed
	{
		...
		if (attrval.value==null || attrval.value.discriminator()==null)
			Except.throw_exception("AttributeTypeNotSet",
					"Attribute " + attrval.name + " Value Has Not Been Set",
		...
		try
		{
			...
			default:
				...
				"Attribute Type (" + attrval.value.discriminator().value() + ") Not Supported",
				"DeviceAttribute.getType()");
			}
		}
		catch(org.omg.CORBA.BAD_PARAM e)
	...
	void setValueAsAny(DeviceAttribute_3DAODefaultImpl att_3) throws DevFailed
	{
		...
		case TangoConst.Tango_DEV_UCHAR:
			att_3.insert(extractUCharArray());
			break;
