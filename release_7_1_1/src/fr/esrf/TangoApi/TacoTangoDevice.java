...
//
// Revision 1.3  2008/06/23 09:17:45  pascal_verdier
// IgnoreCase added on initializeAttributeConfig()
//
// Revision 1.2  2007/12/05 10:18:51  pascal_verdier
// Bug in get_property() fixed.
//
...
public class TacoTangoDevice implements TangoConst, TacoConst
{
	...
	{
		return infoToTango(dev.getInfo());
	}
	//==========================================================================
	//==========================================================================
	...
	{
		String	fullname = devname + "/" + attname;
	...
	AttributeConfig[] get_attribute_config(String[] attrnames) throws DevFailed
	{
		...
		{
			for (AttributeConfig config : att_config)
				if (config.name.equalsIgnoreCase(attrnames[i]))
					tmp_config[i] = config;
		}
	...
	DeviceAttribute[] read_attribute(String[] attrnames) throws DevFailed
	{
		...
		for (int i=0 ; i<attrnames.length ; i++)
		{
			...
			for (int j=0 ; j<att_config.length ; j++)
				if (att_config[j].name.equalsIgnoreCase(attrnames[i]))
					idx = j;
	...
	private void initializeAttributeConfig() throws DevFailed
	{
		...
		for (int i=0 ; i<attrnames.length ; i++)
		{
			...
			{
				if (attr_strconfig[j].equalsIgnoreCase(fullname))
				{
