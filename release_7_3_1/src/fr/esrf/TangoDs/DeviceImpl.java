...
//
		...
		try
		{
			...
			if (Util.getAccessConter()>Util.getPoaThreadPoolMax()-2)
			{
				...
				Except.throw_exception("API_MemoryAllocation",
					Util.instance().get_ds_real_name() +
		...
		{
			Util.decreaseAccessConter();
			Except.throw_exception("API_ExceptionCatched",
				exc.toString(), "DeviceImpl.command_inout");
		}
		Util.decreaseAccessConter();
...
//=========================================================================
	public  AttributeValue[] read_attributes(String[] names) throws DevFailed
	{
		...
		String[] real_names = checkRealNames(names);
		...
		catch(Exception exc)
		{
			...
			exc.printStackTrace();
			Except.throw_exception("API_ExceptionCatched",
				exc.toString(), "DeviceImpl.read_attributes");
		}
 ...
 */
//=========================================================================
	private AttributeValue[] manageReadAttrinbute(String[] real_names, Vector wanted_attr, Vector wanted_w_attr)
		throws DevFailed
	{
		...
		{
			int	idx = ((Integer)(wanted_attr.elementAt(i))).intValue();
			read_attr(dev_attr.get_attr_by_ind(idx));
		}
		// Set attr value for writable attribute
		for (int i=0 ; i<wanted_w_attr.size() ; i++)
		{
			int	idx = ((Integer)(wanted_w_attr.elementAt(i))).intValue();
			AttrWriteType w_type = dev_attr.get_attr_by_ind(idx).get_writable();
 ...
 */
//=========================================================================
	private AttributeValue[] buildAttributeValue(String[] real_names)
		throws DevFailed
	{
		...
		for (int i=0 ; i<real_names.length ; i++)
		{
			...
			if (att.get_value_flag())
			{
				...
				/*
				//========================================================
				//	Not possible (DevEncoded type is not transfered inside an any)
				//========================================================
				...
				*/
				default:
					Except.throw_exception("API_DatTypeNotSupported",
						"Attribute " + att.get_name() + " data type not supported !",
						"Device_2Impl.read_attributes()");	
				}
			}
			else
				Except.throw_exception("API_AttrValueNotSet",
						       "Read value for attribute " + att.get_name() + " has not been updated",
						       "DeviceImpl.read_attributes");
		}	//	End of for ( ; ; )
		return back;
	}
//=========================================================================
/**
 * Retrieve wanted attributes in the device attribute list and
 *	 check (and set) their active flag, before clearing their value set flag
 */
//=========================================================================
	private void sortAttributes(String[] names, Vector wanted_attr, Vector wanted_w_attr)
		throws DevFailed
	{
		for (int i=0 ;i<names.length ; i++)
		{
			Attribute att = dev_attr.get_attr_by_name(names[i]);
			int	att_idx = dev_attr.get_attr_ind_by_name(names[i]);
 ...
 */
//=========================================================================
	private String[] checkRealNames(String[] names)
	{
		if (names.length == 1)
		{
			if (names[0].equals(Tango_AllAttr))
			{
				String[]	real_names = new String[dev_attr.get_attr_nb()];
...
//=========================================================================
//=========================================================================
	private void setNotActive(String[] names) throws DevFailed
	{
...
//
		...
		catch(Exception exc)
		{
			Util.decreaseAccessConter();
			Except.throw_exception("API_ExceptionCatched",
				exc.toString(), "DeviceImpl.write_attributes");
		}
		Util.decreaseAccessConter();
...
//
				...
				{
					Attribute att = dev_attr.get_attr_by_ind((Integer) (attr_list.elementAt(i)));
