...
public class ConnectionDAODefaultImpl implements ApiDefs, IConnectionDAO {
	...
	if (connection.isDevice_is_dbase()) {
		...
		if (connection.url.protocol == TANGO) {
		    ...
		    if (connection.isPrev_failed() && delay < ApiUtil.getReconnectionDelay()) {
		...
		Except.throw_communication_failed(e, "TangoApi_CANNOT_IMPORT_DEVICE",
			"Cannot import " + connection.devname, "Connection.build_connection("
				+ connection.devname + ")");
	    }
	}
    }
	...
	if (connection.access == TangoConst.ACCESS_READ) {
		...
		Except.throw_connection_failed("TangoApi_READ_ONLY_MODE", connection.devname
			+ ".command_inout(" + command + ")  is not authorized !",
