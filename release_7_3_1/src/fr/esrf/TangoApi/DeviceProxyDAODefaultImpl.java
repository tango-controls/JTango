...
public class DeviceProxyDAODefaultImpl extends ConnectionDAODefaultImpl implements ApiDefs,
	...
	if (deviceProxy.url.protocol == TANGO) {
		...
		try {
			...
			if (i == retries - 1) {
			    throw_dev_failed(deviceProxy, e, "device.read_attribute()", false);
			}
		    } else {
			throw_dev_failed(deviceProxy, e, "device.read_attribute()", false);
		    }
	...
	if (deviceProxy.device == null && deviceProxy.devname != null) {
	    checkIfTango(deviceProxy, "command_inout_asynch - build_connection");
	    build_connection(deviceProxy);
	}
	...
	if (request == null) {
		...
		if (except != null) {
			...
			final DevFailed e = DevFailedHelper.extract(any);
			Except.throw_connection_failed(e, "TangoApi_CommandFailed",
