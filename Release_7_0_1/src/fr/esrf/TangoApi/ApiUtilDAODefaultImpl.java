...
public class ApiUtilDAODefaultImpl implements IApiUtilDAO
{
	...
	private static synchronized void create_orb() throws DevFailed
	{
		try
		{
			...
			String	str = checkORBgiopMaxMsgSize();
	...
	//===================================================================
	/**
	 *	Check if the checkORBgiopMaxMsgSize has been set.
	 *	This environment variable should be set in Mega bytes.
	 */
	//===================================================================
	private static String checkORBgiopMaxMsgSize()
	{
		 ...
		 *	buffers immediately rather than keeping them for later reuse.
		 */
		String	str = "20";	//	Set to 16 Mbytes

		//	Check if environment ask for bigger size.
		String	tmp;
		if ((tmp=System.getenv("ORBgiopMaxMsgSize"))==null)
			tmp = System.getProperty("ORBgiopMaxMsgSize");
		if (tmp!=null)
	...
	private static String checkBufferSize(String str)
	{
		...
		long	size = (long)nb_mega * 1024 * 1024;
		long	l = size;
		int	cnt;
		for (cnt = 0 ; l>0 ; cnt++)
			l >>= 1;
