             ...
             implements TangoConst , Runnable
{
    ...
    public void push_structured_event(org.omg.CosNotification.StructuredEvent notification)
    {
		...
		try
		{
        	...
        	{
			System.out.println("Receive heartbeat for " + domain_name + " at " + new Date());
				push_structured_event_heartbeat(domain_name, event_name);
				return;
         	}
                ...
                if (callback != null)
                {
                    ...
                    EventData event_data =
						new EventData(event_callback_struct.device,
                                      ...
                                      attr_value, attr_config, dev_err_list);
	...
	private String buildConstraintExpr(String device_name, String attribute, String event_name, String[] filters)
	{
 	...
 	//===============================================================
	//===============================================================
	private void checkIfAlreadyConnected(DeviceProxy device, String attribute, String event_name, CallBack callback, boolean stateless)
                throws DevFailed
	{
                ...
                throws DevFailed
	{
 	...
 	//===============================================================
	//===============================================================
   public int subscribe_event(DeviceProxy device, String attribute, int event, CallBack callback, String[] filters, boolean stateless)
                throws DevFailed
    {
		//	Set the event name;
		...
		checkIfAlreadyConnected(device, attribute, event_name, callback, stateless);
		...
		//	Build Event CallBack Structure
        EventCallBackStruct new_event_callback_struct = 
			new EventCallBackStruct(device,
									attribute,
									event_name,
...
//		from the tango database
             ...
             extends TimerTask
    {
			...
			while ( callback_structs.hasMoreElements() )
			{
				...
				catch(DevFailed e)
				{
					...
					EventData event_data = new EventData(callback_struct.device,
							callback_key, callback_struct.event_name, null, null, e.errors);
		...
		private void readAttributeAndPush(EventChannelStruct event_channel_struct, EventCallBackStruct callback_struct)
		{
			...
			EventData event_data = new EventData(callback_struct.device,
							domain_name, callback_struct.event_name, da, info, err);
			callback_struct.callback.push_event(event_data);
		}
		//===============================================================
		/**
		...
		private void pushServerNotRespondingException(EventChannelStruct event_channel_struct, EventCallBackStruct callback_struct)
		{
        	try
			{
            	...
            	EventData event_data = new EventData( event_channel_struct. adm_device_proxy,
                                                	  "domain_name" , "event_name" , null , null, errors );
	...
	class PushAttrValueLater extends Thread
	{
		...
		public void run()
		{
			...
			try
			{
				if (event_type==ATT_CONF_EVENT)
					info = device.get_attribute_info_ex(attribute);
				else
			...
			//	And push value
			EventData event_data = new EventData(device, domain_name, event_name, da, info, err);
			callback.push_event(event_data);
		}
	}
