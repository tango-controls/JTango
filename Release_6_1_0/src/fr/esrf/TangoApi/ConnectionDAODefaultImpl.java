...
public class ConnectionDAODefaultImpl implements ApiDefs, IConnectionDAO
{
	...
	{
		connection.url = new TangoUrl(buildUrlName(connection, host, port));
		connection.setDevice_is_dbase(true);
		connection.transparent_reconnection = true;	//	Always true for Database
	...
	public void init(Connection connection, String devname, String host, String port) throws DevFailed {	
		connection.url = new TangoUrl(buildUrlName(connection, host, port, devname));
		connection.devname = connection.url.devname;
		connection.setDevice_is_dbase(false);
    ...
    private void createDevice(Connection connection, String corba_str) throws DevFailed {
      ...
      if (connection.getObj()._is_a("IDL:Tango/Device_3:1.0") == true) {
	...
	private String get_exported_ior(Connection connection)	throws DevFailed
	{
		...
		else
		{
			...
			{
				connection.access = ApiUtil.get_db_obj(connection.url.host, connection.url.strport).checkAccessControl(connection.devname);
		...
		if (connection.device==null)
 		{
			try
			{
				...
				//System.out.println().info("Connected to " + db_corbaloc);
			}
						...
						throws DevFailed
	{
		...
		String	origin = (from_inout_cmd) ?
					new String(connection.devname + ".command_inout(" + command + ")")
					: new String(connection.devname + "." + command + ")");
		...
		{
			desc = new String("Device (" +  connection.devname+
			...
			throws DevFailed
	{
		...
		if (connection.access==TangoConst.ACCESS_READ)
		{
			...
			if (db.isCommandAllowed(connection.devname, command)==false)
			{
		...
		for (int i=0 ; i<retries && done==false ; i++)
		{
			try{
				if (connection.device_2!=null)
					received = connection.device_2.command_inout_2(command, argin.extractAny(), connection.dev_src);
				else
