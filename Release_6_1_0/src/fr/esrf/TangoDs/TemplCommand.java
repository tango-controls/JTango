//+============================================================================
//
// file :               TemplCommand.java
//
// description :        Java source code for the TemplCommand class. This class
//			is a class derived from the Command class. It is used
//			to implement command with the template command method.
//			This class is only for commands without input or output
//			parameters.
//
// project :            TANGO
//
...
//
			...
			if (meth_list.length == 0)
			{
				...
				Except.throw_exception("API_MethodNotFound",
						     new String(mess),
				  	  	     new String("TemplCommand.analyse_method_exe()"));
			}
...
//
			...
			if (meth_list.length == 0)
			{
				...
				Except.throw_exception("API_MethodNotFound",
						     new String(mess),
				  	  	     new String("TemplCommand.analyse_method_state()"));
			}
				...
				for (int j = i + 1;j < meth_list.length;j++)
				{
					if (meth_name.equals(meth_list[j].getName()))
					{
						...
						Except.throw_exception("API_OverloadingNotSupported",
								     new String(mess),
				  	  	  		     new String("TemplCommand.find_method()"));
					}
...
//
		...
		if (type_cl.isArray() == true)
		{
			...
			else
			{
				...
				Except.throw_exception("API_MethodArgument",
						     new String(mess),
				  	  	     new String("TemplCommandIn.get_tango_type()"));
			}
		}
...
//
		...
		catch(IllegalArgumentException e)
		{
			...
			Except.throw_exception("API_MethodArgument",
					       new String(mess),
				  	       new String("TemplCommand.execute()"));
		}
		catch(IllegalAccessException e)
		{
			StringBuffer mess = new StringBuffer("Argument error when trying to invoke method ");
