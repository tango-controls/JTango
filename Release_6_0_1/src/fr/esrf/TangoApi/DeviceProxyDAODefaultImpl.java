...
// $Author$
//
// Revision 3.32  2007/08/07 06:45:42  pascal_verdier
// Methods use_db() and get_db_obj() added.
//
// $Log$
...
public class DeviceProxyDAODefaultImpl extends ConnectionDAODefaultImpl implements ApiDefs, IDeviceProxyDAO
{
	...
	{ 
		if  (!deviceProxy.url.use_db) 
	               Except.throw_non_db_exception("Api_NonDatabaseDevice", 
	                                ...
	                                "DeviceProxy(" + name(deviceProxy) + ").get_db_obj()"); 
				...
				throws DevFailed
	{
		...
		if (deviceProxy.getAdm_dev()==null)
		{
			...
			deviceProxy.setAdm_dev(ds.get_adm_dev(adm_name(deviceProxy)));
			...
			for (int i=0 ; i<retries && done==false ; i++)
			{
				...
				catch(Exception e)
				{
					...
					if ((e.toString().indexOf("org.omg.CORBA.TRANSIENT")>=0 ||
						e.toString().indexOf("org.omg.CORBA.OBJECT_NOT_EXIST")>=0) && i==0)
					{
	...
	{
		if (deviceProxy.url.use_db==false)
			Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").get_class");
	...
	public String[] get_class_inheritance(DeviceProxy deviceProxy) throws DevFailed
	{
		if (deviceProxy.url.use_db==false)
			Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").get_class_inheritance");
				...
				throws DevFailed
	{
		if (deviceProxy.url.use_db==false)
			Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").put_alias()");
				...
				throws DevFailed
	{
		if (deviceProxy.url.use_db==false)
			Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").get_alias()");
				...
				throws DevFailed
	{
		if (deviceProxy.url.use_db==false)
			Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").import_device()");
				...
				throws DevFailed
	{
		...
		if (deviceProxy.url.use_db==false)
			Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").export_device()");
	...
	public void unexport_device(DeviceProxy deviceProxy) throws DevFailed
	{
		...
		if (deviceProxy.url.use_db==false)
			Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").unexport_device()");
	...
	public void add_device(DeviceProxy deviceProxy, DbDevInfo devinfo) throws DevFailed
	{
		...
		if (deviceProxy.url.use_db==false)
			Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").add_device()");
				...
				throws DevFailed
	{
		...
		if (deviceProxy.url.use_db==false)
			Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").get_property_list()");
				...
				throws DevFailed
	{
		if (deviceProxy.url.protocol==TANGO)
		{
			if (deviceProxy.url.use_db==false)
				Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").get_property()");
				...
				throws DevFailed
	{
		if (deviceProxy.url.use_db==false)
			Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").put_property()");
				...
				throws DevFailed
	{
		if (deviceProxy.url.use_db==false)
			Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").delete_property()");
	...
	public String[] get_attribute_list(DeviceProxy deviceProxy) throws DevFailed
	{
		...
		if (deviceProxy.url.protocol==TANGO)
		{
			...
			AttributeInfo[]	ac = get_attribute_config(deviceProxy, wildcard);
				...
				throws DevFailed
	{
		if (deviceProxy.url.use_db==false)
			Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").put_attribute_property()");
				...
				throws DevFailed
	{
		if (deviceProxy.url.use_db==false)
			Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").delete_attribute_property()");
				...
				throws DevFailed
	{
		if (deviceProxy.url.use_db==false)
			Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").get_attribute_property()");
				...
				throws DevFailed
	{
		if (deviceProxy.url.use_db==false)
			Except.throw_non_db_exception("Api_NonDatabaseDevice",
					"Device " + name(deviceProxy) + " do not use database",
					"DeviceProxy(" + name(deviceProxy) + ").delete_attribute()");
	...
	public AttributeInfoEx[] get_attribute_info_ex(DeviceProxy deviceProxy, String[] attnames) throws DevFailed
	{
			...
			if (deviceProxy.url.protocol==TANGO)
			{
				...
				if (deviceProxy.device_2!=null)
				{
					...
					Except.throw_non_supported_exception("TangoApi_IDL_NOT_SUPPORTED",
							...
							new String(deviceProxy.getFull_class_name()+".get_attribute_info_ex()"));
			}
			else
				ac = deviceProxy.taco_device.get_attribute_config(attnames);
		...
		{
			e.printStackTrace();
			throw_dev_failed(deviceProxy, e, "get_attribute_config_ex", true);
			return null;
		}
	}
	//==========================================================================
					...
					throws DevFailed
	{
		...
		try {
			...
			Except.throw_connection_failed(e, "TangoApi_CANNOT_READ_ATTRIBUTE",
							...
							new String(deviceProxy.getFull_class_name()+".read_attribute()"));
			return null;
		}
		catch (Exception e) {
	...
	private void poll_object(DeviceProxy deviceProxy, String objname, String objtype, int period) throws DevFailed
	{
		...
		if (deviceProxy.isCheck_idl() && deviceProxy.getAdm_dev().get_idl_version()<2)	//	Check if IDL support command
			Except.throw_non_supported_exception("TangoApi_IDL_NOT_SUPPORTED",
							"Not supported by the IDL version used by device",
							new String(deviceProxy.getFull_class_name()+".poll_object()"));
	...
	private void remove_poll_object(DeviceProxy deviceProxy, String objname, String objtype) throws DevFailed
	{
		...
		if (deviceProxy.isCheck_idl() && deviceProxy.getAdm_dev().get_idl_version()<2)	//	Check if IDL support command
			Except.throw_non_supported_exception("TangoApi_IDL_NOT_SUPPORTED",
							"Not supported by the IDL version used by device",
							new String(deviceProxy.getFull_class_name()+".remove_poll_object()"));
	...
	public String[] polling_status(DeviceProxy deviceProxy) throws DevFailed
	{
		...
		if (deviceProxy.isCheck_idl() && deviceProxy.getAdm_dev().get_idl_version()<2)	//	Check if IDL support command
			Except.throw_non_supported_exception("TangoApi_IDL_NOT_SUPPORTED",
							"Not supported by the IDL version used by device",
							new String(deviceProxy.getFull_class_name()+".polling_status()"));
		DeviceData	argin = new DeviceData();
	...
	public DeviceDataHistory[] command_history(DeviceProxy deviceProxy, String cmdname, int nb) throws DevFailed
	{
		...
		if (deviceProxy.isCheck_idl() && get_idl_version(deviceProxy)<2)	//	Check if IDL support command
			Except.throw_non_supported_exception("TangoApi_IDL_NOT_SUPPORTED",
							"Not supported by the IDL version used by device",
							new String(deviceProxy.getFull_class_name()+".command_history()"));
		...
		catch(Exception e)
		{
			if (e instanceof DevFailed)
				...
				Except.throw_communication_failed(e.toString(),
						"API has catched a RuntimeException",
						full_class_name+".command_history()");
				*/
		}
		return histo;
	}
	...
	public DeviceDataHistory[] attribute_history(DeviceProxy deviceProxy, String attname, int nb) throws DevFailed
	{
		...
		if (deviceProxy.isCheck_idl() && get_idl_version(deviceProxy)<2)	//	Check if IDL support command
			Except.throw_non_supported_exception("TangoApi_IDL_NOT_SUPPORTED",
							"Not supported by the IDL version used by device",
							new String(deviceProxy.getFull_class_name()+".attribute_history()"));
		...
		catch(Exception e)
		{
			if (e instanceof DevFailed)
				...
				Except.throw_communication_failed(e.toString(),
						"API has catched a RuntimeException",
						full_class_name+".attribute_history()");
				*/
		}
		return histo;
	}
				...
				throws DevFailed
	{
		...
		if (deviceProxy.access==TangoConst.ACCESS_READ)
		{
			...
			if (db.isCommandAllowed(deviceProxy.devname, cmdname)==false)
			{
				...
				Except.throw_connection_failed("TangoApi_READ_ONLY_MODE",
						deviceProxy.devname + ".command_inout_asynch("+cmdname+")  is not authorized !",
		...
		//----------------------------------
		Request request;
		if (deviceProxy.idl_version<2)
			request = deviceProxy.device._request("command_inout");
		else
			request = deviceProxy.device_2._request("command_inout");
			...
			throws DevFailed, AsynReplyNotArrived
	{
		...
		while (((t1-t0)<timeout || timeout==0 ) && argout==null)
		{
			...
			catch (AsynReplyNotArrived na)
			{
				...
				this.sleep(deviceProxy ,ms_to_sleep);
				...
				throws DevFailed
	{
		...
		if (deviceProxy.idl_version<2)
			...
			request = deviceProxy.device_2._request("read_attributes");
		...
		request.send_deferred();
		//	store request reference to read reply later
		int id = ApiUtil.put_async_request(new AsyncCallObject(request, deviceProxy, ATT_R, attnames));
		...
		throws DevFailed, AsynReplyNotArrived
	{
		...
		if (request==null)
			Except.throw_connection_failed(	"TangoApi_CommandFailed",
									...
									new String(deviceProxy.getFull_class_name()+"."+idl_cmd + "_reply()"));
		...
		if (request.poll_response() == false)
		{
			...
			Except.throw_asyn_reply_not_arrived("API_AsynReplyNotArrived",
					new String("Device " + deviceProxy.devname + 
	 ...
	 */
	//==========================================================================
	public void add_logging_target(DeviceProxy deviceProxy, String target_type, String target_name)
				throws DevFailed
	{
		//	Get connection on administration device
		if (deviceProxy.getAdm_dev()==null)
	...
	//===============================================================
	//===============================================================
	private synchronized void sleep(DeviceProxy deviceProxy, long ms)
	{
		try
		{
			wait(ms);
	 ...
	 */
	//==========================================================================
	public int subscribe_event(DeviceProxy deviceProxy, String attr_name, int event, CallBack callback, String[] filters)
                   throws DevFailed
	{
		int	id=0;
		...
		try
		{
			...
			id = event_consumer.subscribe_event(deviceProxy, attr_name.toLowerCase(), event, callback, filters);
		}
		catch(Exception e)
		{
			if (e instanceof DevFailed)
