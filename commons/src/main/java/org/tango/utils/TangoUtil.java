...
public final class TangoUtil {
	...
	if (fields.length == 1) {
	    result = ApiUtil.get_db_obj().get_attribute_alias(fields[0]);
	} else if (fields.length == 2) {
	    result = ApiUtil.get_db_obj().get_alias_device(fields[0]);
	...
	if (attributeName.contains(DBASE_NO)) {
		...
		result = db.get_alias_device(fields[0]) + DEVICE_SEP + fields[1];
	    } else {
		result = attributeName;
	    }
	}
