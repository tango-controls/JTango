...
public final class ArrayUtils {
	...
	if (array2D.getClass().isArray()) {
		...
		if (array2D.getClass().getComponentType().isArray()) {
			...
			for (int x = 0; x < lengthLineX; x++) {
			    Array.set(array, idxArray++, Array.get(line, x));
			}
		    }
	...
	if (array.getClass().isArray()) {
		...
		for (int y = 0; y < dimY; y++) {
			...
			try {
			    final Object value = Array.get(array, idx1D++);
			    Array.set(line, x, value);
			} catch (final ArrayIndexOutOfBoundsException e) {
			    DevFailedUtils.throwDevFailed("array does not have a correct size - should be " + dimX
				    * dimY + " but is " + Array.getLength(array));
			}
		    }
		}
	    } else {
