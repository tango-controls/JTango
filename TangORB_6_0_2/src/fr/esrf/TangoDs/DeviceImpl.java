 ...
 *
 * @exception DevFailed Re-throw of the exception thrown during the creation
 *                      of the multi-attribute object.
 ...
 */
	...
	public DeviceImpl(DeviceClass cl,String d_name) throws DevFailed
	{
		...
		desc = new String("A Tango device");
		device_state = DevState.UNKNOWN;
		device_status = new String("Not Initialised");
...
//
		...
		if (Util.instance()._UseDb == true)
		{
			...
			catch (DevFailed e)
			{
				...
				Except.throw_exception("API_DatabaseAccess",
				                       o.toString(),
 ...
 */
	...
	public synchronized Any command_inout(String in_cmd, Any in_any) throws DevFailed
	{
...
//
		...
		Any out_any = device_class.command_handler(this,in_cmd,in_any);
...
//
//--------------------------------------------------------------------------
	public Any command_inout_2(String in_cmd, Any in_data, DevSource source)
			throws DevFailed, SystemException
	{
		...
		try
		{
			...
			boolean	found = false;
			Vector	poll_list = get_poll_obj_list();	//	PollObj
			...
			if (found == false)
			{
				...
				if (poll_period == 0)
				{
					Except.throw_exception("API_CmdNotPolled",
							"Command " + in_cmd + " not polled",
							"Device_2Impl.command_inout");
				}

				found = false;
				Vector	napc = get_non_auto_polled_cmd();
			...
			if (polled_cmd==null)
				Except.throw_exception("API_CommandNotPolled",
						"Command not found.",
						"Device_2Impl.command_inout");
...
//
//--------------------------------------------------------------------------
	public DevCmdHistory[] command_inout_history_2(String command, int n)
			throws DevFailed,  SystemException
	{
		...
		// Check that the command is polled
		PollObj	polled_cmd = null;
		Vector	poll_list = get_poll_obj_list();
		for (int i=0 ; i<poll_list.size() ; i++)
		{
			PollObj	poll_obj = (PollObj)poll_list.elementAt(i);
			if ((poll_obj.get_type() == Tango_POLL_CMD) &&
		    	poll_obj.get_name().equals(cmd_str))
			{
				polled_cmd = poll_obj;
			}
		}
		...
		{
			Except.throw_exception("API_NoDataYet",
						"No data available in cache for command " + cmd_str,
						"Device_2Impl.command_inout_history_2");
		}
		// Set the number of returned records
		int	in_buf = polled_cmd.get_elt_nb_in_buffer();
		if (n > in_buf)
			n = in_buf;
 ...
 *
 * It's the master method executed when the administration device name is requested
 * via a CORBA attribute. It updates the device black-box and return the
 * administration device name
 *
 ...
 * via a CORBA attribute. It updates the device black-box and return the
 * device state. This method calls the <i>dev_state</i> device method but 
...
//
// Return data to caller. If the dev_state throw an exception, catch it and do 
 ...
 * via a CORBA attribute. It updates the device black-box and return the
 * device state. This method calls the <i>dev_status</i> device method but 
 * catch all the execption and does not re-throw them because exception can't
 * be thrown to a client for CORBA attribute
 *
...
//
// Return data to caller. If the dev_status method throw exception, catch it
...
//
		...
		int nb_cmd = device_class.get_command_list().size();
 ...
 */
//===========================================================================
	public DevCmdInfo_2 command_query_2(String command)
		throws DevFailed, SystemException
	{
		...
		for (int i=0 ; i<nb_cmd ; i++)
		{
			Command	cmd = (Command)device_class.get_command_list().elementAt(i);
...
//
			Except.throw_exception("API_CommandNotFound",
					     "Command " + command + " not found",
					     new String("DeviceImpl.command_query()"));
		}
...
//
// This method returns a reference to a AttributeConfig array with one AttributeConfig
 ...
 * @return An array of AttributeConfig object. One AttributeConfig object is initialised
 * for each wanted attribute
 * @exception DevFailed Thrown if one of the attribute does not exist.
 * Click <a href="../../tango_basic/idl_html/Tango.html#DevFailed">here</a> to read
 * <b>DevFailed</b> exception specification
 */
...
//
		...
		int nb_dev_attr = dev_attr.get_attr_nb();
		if (nb_dev_attr == 0)
		{
			Except.throw_exception("API_AttrNotFound",
					     "The device does not have any attribute",
...
//
//--------------------------------------------------------------------------
	public AttributeConfig_2[] get_attribute_config_2(String[] names)
		throws DevFailed, SystemException
	{
 ...
 * database with the new value. This method allows to update only the optional
 * attribute properties.
 *
 * @param new_conf The attribute(s) new configuration list. There is one 
 * AttributeConfig object for each attribute with configuration to be updated
 * @exception DevFailed Thrown if one of the attribute does not exist.
 * Click <a href="../../tango_basic/idl_html/Tango.html#DevFailed">here</a> to read
 * <b>DevFailed</b> exception specification
 */
...
//
			...
			for (int j = 0;j < nb_dev_attr;j++)
			{
				...
				{
					dev_attr.get_alarm_list().addElement(new Integer(j));
				}
			}		
...
//
		...
		for (i = 0;i < nb_dev_attr;i++)
		{
			...
			{
				dev_attr.get_alarm_list().addElement(new Integer(i));
			}
		}
 ...
 * This method read attributes value and return them to the caller. A date and 
 * a quality factor is added to each attribute.
 *
 * @param names The attribute name list
 * @return An array of AttributeValue object. One AttributeValue object is initialised
 * for each wanted attribute
 * @exception DevFailed Thrown if one of the attribute does not exist or if a problem
 * occurs during attribute reading.
 * Click <a href="../../tango_basic/idl_html/Tango.html#DevFailed">here</a> to read
 * <b>DevFailed</b> exception specification
 */
//=========================================================================
	public synchronized AttributeValue[] read_attributes(String[] names) throws DevFailed
	{
		Util.out4.println("DeviceImpl.read_attributes arrived synchronized");
...
//
		...
		if (nb_names == 1)
		{
			if (names[0].equals(Tango_AllAttr) == true)
			{
...
//
		...
		Vector wanted_attr = new Vector();
		Vector wanted_w_attr = new Vector();

		for (i = 0;i < nb_names;i++)
		{
			int j = dev_attr.get_attr_ind_by_name(real_names[i]);
			if (dev_attr.get_attr_by_ind(j).get_writable() == AttrWriteType.READ_WRITE)
			{
				wanted_w_attr.addElement(new Integer(j));
				wanted_attr.addElement(new Integer(j));
				dev_attr.get_attr_by_ind(((Integer)(wanted_attr.lastElement())).intValue()).set_value_flag(false);
			}
			else
			{
				if (dev_attr.get_attr_by_ind(j).get_writable() == AttrWriteType.WRITE)
				{
					wanted_w_attr.addElement(new Integer(j));
				}
				else
...
//
		...
		{
			read_attr(dev_attr.get_attr_by_ind(((Integer)(wanted_attr.elementAt(i))).intValue()));
		}
...
//
		...
		{
			AttrWriteType w_type = dev_attr.get_attr_by_ind(((Integer)(wanted_w_attr.elementAt(i))).intValue()).get_writable();
			if ((w_type == AttrWriteType.READ_WITH_WRITE) ||
		    	    (w_type == AttrWriteType.WRITE)) 
				dev_attr.get_attr_by_ind(((Integer)(wanted_w_attr.elementAt(i))).intValue()).set_value();
		}
...
//
		...
		for (i = 0;i < nb_names;i++)
		{
			...
			{			
				...
				Except.throw_exception("API_AttrValueNotSet",
						       o.toString(),
						       "DeviceImpl.read_attributes");
			}
			else
			{
				...
				if ((w_type == AttrWriteType.READ) ||
			    	    (w_type == AttrWriteType.READ_WRITE) ||
				...
				case Tango_DEV_STATE :
					...
					break;
				default:
					Except.throw_exception("API_DatTypeNotSupported",
						"Attribute " + real_names[i] +
 ...
 * a DevSource parameter.
 * This method read attributes value and return them to the caller. A date and 
 * a quality factor is added to each attribute.
 *
 * @param names The attribute name list
 * @param source could be DevSource.CACHE, DevSource.CACHE_DEV or DevSource.DEVICE.
 * @return An array of AttributeValue object. One AttributeValue object is initialised
 * for each wanted attribute
 * @exception DevFailed Thrown if one of the attribute does not exist or if a problem
 * occurs during attribute reading.
 ...
 */
//=========================================================================
	public AttributeValue[] read_attributes_2(String[] names, DevSource source)
		throws DevFailed
	{
		...
		AttributeValue[] back = null;
			...
			try
			{
				...
				Vector	non_polled = new Vector();	//	String
				if (all_attr == false)
				{
					for (i=0 ; i<real_names.length ; i++)
					{
				...
				if (non_polled.size() != 0)
				{
					...
					for (i=0 ; i<non_polled.size() ; i++)
					{
						...
						Attribute	att = dev_attr.get_attr_by_name(non_polled_name);

						poll_period.add(new Integer(att.get_polling_period()));

						if (att.get_polling_period() == 0)
						{
							Except.throw_exception("API_AttrNotPolled",
										"Attribute " + non_polled_name + " not polled",
										"Device_2Impl.read_attributes");
						}
						else
						{
							boolean	found = false;
							Vector	napa = get_non_auto_polled_attr();

							for (j=0 ; j<napa.size() ; j++)
							{
								String	napa_str =
									((String)napa.elementAt(j)).toLowerCase();
					...
					for (i=0 ; i<non_polled.size() ; i++)
					{				
						send.lvalue[0] = ((Integer)poll_period.elementAt(i)).intValue();
				...
				{		
					...
					for (j=0 ; j<poll_list.size() ; j++)
					{
						PollObj	poll_obj = (PollObj)poll_list.elementAt(j);
						if (poll_obj.get_type()==Tango_POLL_ATTR  &&
			    	    		poll_obj.get_name().toLowerCase().equals(
												real_names[i].toLowerCase()))
						{
					...
					{
						Except.throw_exception("API_NotUpdatedAnyMore",
								"Data in cache for attribute " + real_names[i] +
								" not updated any more",
								"Device_2Impl.read_attributes");
					}
					...
					// data buffer.
					AttributeValue	att_val = polled_attr.get_last_attr_value();
					back[i] = att_val;
				}
			}
			catch(DevFailed e)
...
//--------------------------------------------------------------------------
	...
	public DevAttrHistory[] read_attribute_history_2(String name, int n)
			throws DevFailed, SystemException
	{
		...
		for (int i=0 ; i<nb_poll ; i++)
		{
			PollObj	poll_obj = (PollObj)poll_list.elementAt(i);
			if ((poll_obj.get_type() == Tango_POLL_ATTR) &&
		    		poll_obj.get_name().equals(attr_str))
			{
				polled_attr = poll_obj;
				break;
			}
		...
		if (polled_attr==null)	//	NOT found
			Except.throw_exception("API_AttrNotPolled",
					"No data available in cache for attribute " + attr_str,
		...
		// Set the number of returned records
		int	in_buf = polled_attr.get_elt_nb_in_buffer();
		if (n > in_buf)
			n = in_buf;

		// return attribute value history
		DevAttrHistory[] histo =
			polled_attr.get_attr_history(n, att.get_data_type());
		return histo;
	}
 ...
 *
 * @param values The attribute(s) new set value. There is one 
 * AttributeValue object for each attribute to be set
 * @exception DevFailed Thrown if one of the attribute does not exist.
 * Click <a href="../../tango_basic/idl_html/Tango.html#DevFailed">here</a> to read
 * <b>DevFailed</b> exception specification
 */
	public synchronized void write_attributes(AttributeValue[] values) throws DevFailed
	{
...
//
		...
		Vector updated_attr = new Vector();

		int i = 0;
		for (i = 0;i < nb_updated_attr;i++)
		{
			updated_attr.addElement(new Integer(dev_attr.get_attr_ind_by_name(values[i].name)));
		}
...
//
		...
		for (i = 0;i < nb_updated_attr;i++)
		{
			if ((dev_attr.get_attr_by_ind(((Integer)(updated_attr.elementAt(i))).intValue()).get_writable() == AttrWriteType.READ) ||
			    (dev_attr.get_attr_by_ind(((Integer)(updated_attr.elementAt(i))).intValue()).get_writable() == AttrWriteType.READ_WITH_WRITE))
			{
				StringBuffer o = new StringBuffer("Attribute ");
				o.append(dev_attr.get_attr_by_ind(((Integer)(updated_attr.elementAt(i))).intValue()).get_name());
				o.append(" is not writable");

				Except.throw_exception("API_AttrNotWritable",
						       o.toString(),
						       "DeviceImpl.write_attributes");
			}
		}
...
//
		...
		for (i = 0;i < nb_updated_attr;i++)
		{
			try
			{
				dev_attr.get_w_attr_by_ind(((Integer)(updated_attr.elementAt(i))).intValue()).set_write_value(values[i].value);
			}
			catch (DevFailed ex)
			{
				for (int j = 0;j < i;j++)
					dev_attr.get_w_attr_by_ind(((Integer)(updated_attr.elementAt(j))).intValue()).rollback();
 ...
 */
	...
	{
		if ((device_state.value() == DevState._ON) ||
		    (device_state.value() == DevState._ALARM))
		{
...
//
			...
			int nb_wanted_attr = attr_list.size();
			if (nb_wanted_attr != 0)
			{
...
//
				...
				for (i = 0;i < nb_wanted_attr;i++)
				{
					Attribute att = dev_attr.get_attr_by_ind(((Integer)(attr_list.elementAt(i))).intValue());
 ...
 * device_status field. If the device state is ALARM, alarm messages are
 ...
 */
	...
	{
		if ((device_state.value() == DevState._ALARM) ||
		    (device_state.value() == DevState._ON))
		{
			StringBuffer alarm_status = new StringBuffer(device_status);
 ...
 * Return device POA
 *
 * Return a reference to the device POA. This method is necessary for
 * the CORBA object implicit activation by the _this method
 *
 ...
 * Get device status.
 *
 * Return the device device_status field. This method does the same thing than the
 * default dev_status method.
 *
 ...
 * Get device state.
 *
 * Return the device device_state field. This method does the same thing than the
 * default dev_state method.
 *
 * @return Device state
 */
  ...
  public void init_logger() 
  {
    try {
      ...
      if (level_set_from_cmd_line) {
        ...
        } catch (DevFailed df) { /* ignore exception */ }
      }
      ...
      Logging.LoggingProperties properties = Logging.instance().get_logging_properties(get_logger(), util.get_database());
      if (properties == null) {
      ...
      if (properties.logging_targets != null) {
        ...
        for (int i = 0; i < properties.logging_targets.length; i++) {
          try {
            Logging.instance().add_logging_target(get_logger(), properties.logging_targets[i]); 
          } catch (DevFailed e) { /* ignore any exception */ }
        }
      }
 ...
 */
  public void start_logging () 
  {
	...
	//==========================================================
	/**
	 *	This method check that a comamnd is supported by 
			...
			if (cmd.get_name().toLowerCase().equals(cmd_name))
			{
				if (cmd.get_in_type() != Tango_DEV_VOID)
				{
					Except.throw_exception("API_IncompatibleCmdArgumentType",
						"Command " + cmd_name + " cannot be polled because it needs input value",
	...
	void set_non_auto_polled_cmd(String[] s)
	{
		for (int i=0 ; i<s.length ; i++)
			ext.non_auto_polled_cmd.add(s[i]);
	}
	//==========================================================
	//==========================================================
	...
	{
		for (int i=0 ; i<s.length ; i++)
			ext.non_auto_polled_attr.add(s[i]);
	}
	//==========================================================
	//==========================================================
	...
	//==========================================================
	/**
	 *	This method check return the polled object for
	 *	input type and name.
	 *
	 ...
	 */
	//==========================================================
	synchronized PollObj get_polled_obj_by_type_name(int obj_type, String obj_name)
			throws DevFailed
	{
		...
		for (int i=0 ; i<po_list.size() ; i++)
		{
			...
			if (polled_obj.get_type_i() == obj_type)
				if (polled_obj.get_name_i() == obj_name)
					return polled_obj;
		}
	
		//	If not found throws exception
		Except.throw_exception("API_PollObjNotFound",
					obj_name + " not found in list of polled object",
					"DeviceImpl.get_polled_obj_by_type_name");
		return null;
	}
	//==========================================================
	//==========================================================
